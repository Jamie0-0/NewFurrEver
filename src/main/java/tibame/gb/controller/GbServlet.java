...
public class GbServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<GbOrderVO> gbOrderVOs = service.getAllGbOrdersWithGbDetails();
		...
		for (GbAndProductVO gbAndProduct : productList) {
			...
			productObject.addProperty("p_status", product.getP_status()); //商品狀態 0已上架 1未上架
			//以下圖片 //將圖片轉換為Base64編碼
        ...
        for (GbOrderVO gbOrderVO : gbOrderVOs) {  
        	JsonObject gbOrderObject = new JsonObject();
        	gbOrderObject.addProperty("GbPNum", gbOrderVO.getGb_p_num());
        	gbOrderObject.addProperty("GbOrderId", gbOrderVO.getGb_order_id());
        	gbOrderArray.add(gbOrderObject);
        }
        jsonObject.add("GbOderData", gbOrderArray);
