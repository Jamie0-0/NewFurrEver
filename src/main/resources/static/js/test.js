...
function addArt(data) {
	if (data != null) {
		for (let i = 0; i < data.length; i++) {
			...
			$("button.blog-button").eq(i).attr("artId", data[i].art_id);
			...
			$("img.pic-content").eq(i).attr("artId", data[i].art_id);
			...
			$("i.fa-heart").eq(i).on("click", function(e){
				...
				fetch(`artLike?artId=${data[i].art_id}&uid=3`)    // 登入的人 暫定uid=3
			...
			$("img.pic-content").eq(i).attr("src", "forum" + "?getPic&art_id=" + artId);  // 封面圖綁src
			...
			// ComCount
			$.ajax({
				url: `artComCount?comArtId=${data[i].art_id}`,           // 資料請求的網址
				type: "GET",
				dataType: "json",
